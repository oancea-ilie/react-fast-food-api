        ...
        if (allObj) {
            for (let p of allObj) {
                if (p.name == newObj.name) {
                    throw new Error(
                        'Acest Categorie exista deja in baza de date!'
                    );
                }
            }
        }
...
const update = asyncHandler(async (req, res) => {
    ...
    if (obj) {
        ...
        throw new Error(
            `Nu s-a gasit Category cu acest ID pentru a putea face Update!`
        );
    }
});
...
const distroy = asyncHandler(async (req, res) => {
    ...
    if (obj) {
        ...
        throw new Error(
            'Nu s-a gasit Category cu acest ID pentru a putea fi sters!'
        );
    }
});
